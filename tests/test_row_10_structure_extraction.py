...
GUIDELINES_DIR = Path(
    os.environ.get("CARDIO_GRAPH_GUIDELINES_DIR", DATA_DIR / "guidelines")
)
EXPECTED_JSON_PATH = Path(
    os.environ.get(
        "CARDIO_GRAPH_EXPECTED_ROW10_PATH",
        PROJECT_ROOT / "tests" / "expected_row_10_structure.json",
    )
)
DOC_TABLE_JSONS = [
    GUIDELINES_DIR / "docling" / "pdf_pages" / "_62" / "tables" / "table_000.json",
    GUIDELINES_DIR / "docling" / "pdf_pages" / "_63" / "tables" / "table_000.json",
]
...
class Row10StructureExtractionTests(unittest.TestCase):
    def setUp(self):
        missing_tables = [str(path) for path in DOC_TABLE_JSONS if not path.is_file()]
        if missing_tables:
            self.skipTest(
                "Missing Docling table JSONs: "
                + ", ".join(missing_tables)
                + ". Set CARDIO_GRAPH_GUIDELINES_DIR."
            )
        if not EXPECTED_JSON_PATH.is_file():
            self.skipTest(
                "Missing expected structure JSON: "
                + str(EXPECTED_JSON_PATH)
                + ". Set CARDIO_GRAPH_EXPECTED_ROW10_PATH."
            )
    ...
    def _build_actual_summary(self, row_text, concepts):
        ...
        for concept in concepts:
            ...
            if concept.role in {"ClinicalCondition", "ClinicalParameter"}:
                rule_payload["conditions"].append(entry)
            elif concept.role in {"Procedure", "Medication"}:
                rule_payload["actions"].append(entry)
